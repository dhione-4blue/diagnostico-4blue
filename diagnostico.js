/* =====================================================================
   ...
   Fluxo: setup -> avaliação -> 7 pilares -> dados (lead) ->
          [qualificado] levantada de mão -> landing ILU/MDL -> resultado
          [não qualificado] resultado (com botão combo)
   Inclui motor de personalização adaptativo ("IA própria" por regras).
   ===================================================================== */
"use strict";
...
/* =================== ESTADO =================== */
const S = {
  step:'setup', pillarIdx:0, submitting:false, submitted:false,
...
/* =================== MOTOR DE PERSONALIZAÇÃO ("IA própria") =================== */
// Gera textos adaptados às respostas de cada lead (sem chamadas externas).
function insights(r){
  const sorted = r.pillarScores.slice().sort((a,b)=>a.score-b.score);
  const weak = sorted.slice(0,2);                 // 2 pilares mais frágeis
  const strong = sorted[sorted.length-1];          // pilar mais forte
  const financeScore = r.pillarScores[0].score;    // Finanças = índice 0
  const minOther = Math.min.apply(null, r.pillarScores.slice(1).map(p=>p.score));
  const financeIsWorst = financeScore <= minOther; // empate vai para ILU
  const nome = firstName();

  // Saudação adaptada ao nível
  const lvlMsg = {
  ...
  // Lista "o que melhorar"
  const improveList = weak.concat(sorted[2] ? [sorted[2]] : []).map(p=>({
    name:p.name, icon:p.icon, text:p.improve, score:p.score
  }));

  return { weak, strong, financeIsWorst, lvlMsg, focusMsg, strongMsg, improveList };
}

/* =================== QUALIFICAÇÃO E ROTEAMENTO =================== */
const BELOW = CFG.BELOW_60K_RANGES || ['Até 30mil/mês','30 a 60mil/mês','Nada / Não tenho empresa'];
const LEADERSHIP = CFG.LEADERSHIP_ROLES || ['Dono / Sócio','Gerente / Coordenador / Supervisor'];
function isAbove60k(){ return !BELOW.includes(S.lead.revenueRange); }
function isLeadership(){ return LEADERSHIP.includes(S.lead.jobTitle); }
function isQualified(){ return isAbove60k() && isLeadership(); }

// Tag de controle (base) — espelha o backend
function baseTag(r){
...
function buildPayload(r, extra){
  const p = {
    ...
    'Tag': baseTag(r), 'Solucao Sugerida': decideSolution(r),
  };
...
// Levantada de mão (clicou no CTA do ILU/MDL): adiciona a tag e segue ao diagnóstico
  ...
  return `<div class="step stack">
    ...
    <div class="card grid2">
      <div class="field" ${S.businessType==='other'?'style="grid-column:1/-1"':''}>
        <label class="lbl">Tipo de Negócio</label>
...
function iluView(){
  ...
  return `<div class="step lp">
    ...
    <button class="link-skip" data-act="decline">Não quero conhecer a solução agora — ver meu diagnóstico completo</button>
  </div>`;
}
